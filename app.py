...
col1.metric("Total Load", int(df['Total_Load'].iloc[-1]), delta_color ="normal", "+50" )
col2.metric("Net Intake", int(df['Net_Intake'].iloc[-1]), delta_color ="normal", "-1" )
col3.metric("Growth Rate %", round(df['Growth_Rate'].iloc[0], 2), "%")
col4.metric("Backlog Active", int(df['Backlog'].sum()), delta_color ="normal", "+5" )
